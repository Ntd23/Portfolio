...
@section('card-title')
  <div class="container d-flex justify-content-between align-items-sm-start">
    <h4 class="card-title ml-2">New About</h4>
  </div>
@endsection
@section('content')
  <div class="container">
    <form class="row" action="{{ route('admin.about.create') }}" method="POST" enctype="multipart/form-data">
      @csrf
      <div class="col col-6">
      ...
      <div class="col">
        ...
        <input class="form-control" type="file" onchange="PreviewImage();" id="uploadImage" accept="image/*"
          name="avatar">
        <img id="uploadPreview" style="width: 50%" class="mt-6">
        @error('avatar')
          <div class="text-danger">{{ $message }}</div>
        @enderror
      </div>
      ...
      <div class="col col-6 mt-4 m-auto pb-4 pt-4">
        <button type="submit" class="btn btn-block btn-success">New About</button>
      </div>
    </form>
  </div>
@endsection
